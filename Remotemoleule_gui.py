...
def molecule_testing(N1, L1, N2, L2, N3, L3):
    ...
    #TODO fix this! equilibrium is 0 must have the actual file name
    with open('water-equil.csv', encoding='UTF-8') as f:
        for row in f:
            print(row)
    ...
    def getNs():
        file = open('water-potential.csv', encoding='UTF-8')
        reader = csv.reader(file)
        lines = len(list(reader))
        totalN_2 = int(N1_1) * int(N2_1) * int(N3_1)
        if lines == totalN_2:
    ...
    def hi():
        ...
        print(r)
        hola = open('water-potential.csv', encoding='UTF-8')
        for hello in hola:
    ...
    deltaQ1 = int(L1_2) / float(N1_2)
    deltaQ2 = int(L2_2) / float(N2_2)
    deltaQ3 = int(L3_2) / float(N3_2)
    with open('water-potential.csv', encoding="utf-8") as a:
        for x in a:
    ...
    pes.setN([int(N1),int(N2),int(N3)])
    pes.setNpts(int(N1)*int(N2)*int(N3))
    n = 0
    for i in range(int(N1_2)):
        for j in range(int(N2_2)):
            for k in range(int(N3_2)):
