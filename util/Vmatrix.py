...
def Vab(d, NValue, LValue, VType, VModel, deltax, pes, dimensionCounterArray):
...
def VBlockCalc(dimensions, NValue, LValue, VType, VModel, pes, blockX, blockY):
    ...
    for alpha in range(0+NValue[0]*blockX, NValue[0]+NValue[0]*blockX):
        for beta in range(0+NValue[0]*blockY, NValue[0]+NValue[0]*blockY):
            ...
            blockHolder[alpha0start, beta0start] = (Vab(dimensions, NValue, LValue, VType, VModel, 0, pes, counter))
            beta0start += 1
        alpha0start += 1
        beta0start = 0
    return blockHolder
...
def VMatrixCalc(params, D):
    ...
    for coords in optBlockCoords:
        paramz.append((dimensions, NValue, LValue, VType, VModel, pes, coords[0], coords[1]))

    #Disable multi for debug
    p = mp.Pool(16)
    print("Pool go V")
    ...
    #    blocks.append(VBlockCalc(*paramz[i]))  
